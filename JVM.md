...
分为`标记`和`清除`两个阶段，标记过程如引用计数算法。标记完成后统一回收所有被标记的对象。标记-清除算法是最基础的算法，其他收集算法都是基于这个思路对其的补充。
...
标记-整理算法的标记过程和标记-清除一样，但后续步骤不是直接对对象进行回收，而是让所有存活的对象向一端移动，然后直接清理掉边界意外的内存。
...
虚拟机给每个对象定义了一个对象年龄计数器，如果对象在Eden初始并经过第一次Minor GC并且能被Survivor容纳，会移到Survicor空间，并设置年龄为1，经历多次回收之后，年龄到达一定程度（默认15）会被移到老年代。