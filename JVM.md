...
虚拟机栈描述的是Java方法执行的内存模型：每个方法被执行的时候都会同时创建一个栈帧（Stack Frame）用于存储局部变量表，操作栈，动态链接，方法出口等信息。每个方法调用直至执行完毕对应一个栈帧从虚拟机栈入栈到出栈的一个过程。
...
虚拟机遇到一条new指令指，首先检查这个指令的参数是否能在常量池的符号引用，并检查这个符号引用代表的类是否被加载，解析和初始化。如果没有则会先加载相应的类。

类加载通过后，虚拟机就会为新生对象分配内存。对象所需内存大小在类加载完成后便可完全确定。分配完空间后，虚拟机要对对象进行必要的设置，如对象是哪个类的实例，如何找到类的元数据信息，对象的哈希码，对象GC分代年龄等信息。这些信息放在对象头中。这样虚拟机新对象已经产生了。
...
如果java堆中内存是规整的，使用过和未使用的分开存放，中间使用指针作为分界点，那分配内存只需要移动相应内存大小的指针位置，就可以完成分配。

#### 空闲列表

如果java堆内存不是规整的，虚拟机会维护一份列表记录哪些内存块是可以使用的，分配内存时，会从列表中找出一块空间足够的内存分配给对象。

而java堆是否规整又由所采用的垃圾收集器是否带有压缩整理功能，因此使用Serial,ParNew等带Compact过程的收集器时，采用指针碰撞，而使用CMS这种基于Mark-Sweep算法的收集器时，通常采用空闲列表。

### 分配并发问题

如正在给对象A分配内存，指针还没来得及修改，对象B又同时使用了原来的指针。

- CAS + 失败重试

- 本地线程缓冲

  每个线程在java堆预先分配一小块内存，哪个线程需要内存就在哪个线程的TLAB上分配，只有TLAB用完并需要分配新的TLAB时才需要同步锁定。是否使用TLAB可以通过`-XX : +/-UseTLAB`参数设定

### 对象内存布局

HotSpot虚拟机中内存分为对象头，实例数据，对齐补充。

- 对象头

  分为两部分，一部分用于存储对象自身运行时数据，如哈希码，GC分代年龄，锁状态标志，线程持有的锁，偏向线程ID，偏向时间戳等。另一部分是类型指针，指向它的类元数据。来确定是哪个类的实例。如果是数组则还有记录数据长度的数据。
...
唯一作用是确定文件是否被虚拟机接受。占4个字节，放在每个Class文件的开始。

很多文件都以魔数进行身份识别。因为文件后缀名可以随意改动。

紧接着魔数的四个字节存储的是Class文件的版本号：第5，6个字节是次版本号，第7，8个是主版本号。